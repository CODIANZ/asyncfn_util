...
```refcon``` は非同期関数 ```asyncfunc()``` がコールバック関数```callback```を呼び出す際にそのまま透過して引数に透過します。そして、コールバック関数は非同期関数を呼び出した際の ```refcon``` を得られます。一般的にはこの ```refcon``` に何らかのオブジェクトインスタンスへのポインタを渡すことが多いと思います。```refcon``` は Apple で古くから使われている用語ですが、他の API や Framework でもこの ```refcon``` と同様の意味を持つ変数が存在します。（Reference Content? Reference Context?）

ここで、C++であればラムダ式を使いたいところなので、下記のようにラムダ式を書いてみると問題なくコンパイルでき、動作します。
...
ここで、```const int n = 10``` 外に出してみて、ラムダ導入子 ```[=]``` を追加したくなりますよね？またはコールバック関数が呼び出されたらメンバ関数を呼び出すとか。。。
...
しかし、残念ながら、上記のコードはコンパイルエラーになってしまいます。（理由については、割愛しますが、自分で```struct```を使って外部変数をキャプチャするような関数オブジェクトを作ってみると納得できると思います。）
